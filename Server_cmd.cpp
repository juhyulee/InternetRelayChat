...
void	Server::commandJoin(std::vector<std::string> token, int paramcnt, Client * user, int fd) {
	...
	// error::client exceed max channel cnt
		...
		sendMessage(ERR_TOOMANYCHANNELS(user->getNickname(), "JOIN"), fd);
		return ;
	}
	// error::
	...
	// Success:: exist channel
		...
		broadcastChannelMessage(RPL_JOIN(user->getNickname(), channel_name), fd);
		//위 fd map으로 바꿔야 함...
		if (ch->getChannelTopic() != "") {
			//RPL_TOPIC
			broadcastChannelMessage(RPL_TOPIC(user->getNickname(), channel_name, ch->getChannelTopic()), fd);
		...
		broadcastChannelMessage(RPL_NAMREPLY(user->getNickname(), "=", channel_name, user->getPrefix()), fd);
		broadcastChannelMessage(RPL_ENDOFNAMES(user->getNickname(), channel_name), fd);
	}
	return ;
}
...
void	Server::commandNick(std::vector<std::string> token, int paramcnt, Client * user, int fd) {
	if (paramcnt != 1) {
		sendMessage(ERR_NEEDMOREPARAMS(user->getNickname(), (std::string)"NICK"), fd);
		return ;
	}
